...
# DBTITLE 1,Inline Code Generation: Read Parquet Files from an GitHub Repo
# MAGIC %md
# MAGIC In the below cell, use option + shift + space on MacOS or control + shift + space on Windows
...
# Read parquet file into a DataFrame and display the first 5 rows
...
# MAGIC %md
# MAGIC <b>Prompt:</b> Generate code to rename FR to FRANCE in the country column

# COMMAND ----------

# Generate code to rename FR to FRANCE in the country column
...
# Assuming you have a DataFrame called `df` with columns `country`, `canal`, and `churned_users`

# Perform the necessary data aggregation
aggregated_data = df.groupby(['country', 'canal'])['churned_users'].sum().unstack()

# Plot the stacked bar chart
aggregated_data.plot(kind='bar', stacked=True)

# Customize the chart labels and titles
plt.xlabel('Country')
plt.ylabel('Number of Churned Users')
plt.title('Number of Churned Users by Country and Canal')
